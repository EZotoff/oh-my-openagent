...
const TOAST_MESSAGE = [
  ...
  "Do NOT use Sisyphus with GPT (except GPT-5.4, GPT-5.5, GPT-5.6 Sol, and GPT-6 models, which have GPT-native prompt support).",
...
function getNativeSisyphusGptVariant(model: { providerID: string; modelID: string }): string | undefined {
  if (isGpt5_5Model(model.modelID) || isGpt6Model(model.modelID)) return "medium"
